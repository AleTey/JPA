...
public class PrestamoJpaController implements Serializable {
    ...
    public List<Prestamo> findPrestamoByIsbn(Long isbn) {
        EntityManager em = null;
        try {
            em = getEntityManager();
            String jpql = "SELECT p FROM Prestamo p WHERE p.libro.isbn = :isbn";
            TypedQuery<Prestamo> query = em.createQuery(jpql, Prestamo.class);
            query.setParameter("isbn", isbn);
            List<Prestamo> prestamos = query.getResultList();
            return prestamos;
        }catch(EntityNotFoundException enf) {
            throw new EntityNotFoundException("No se encontraron prestamos con el id: " + isbn);
        } catch (Exception e) {
            throw e;
        }
    }
